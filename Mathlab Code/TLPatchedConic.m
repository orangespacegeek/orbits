...
r_moon=1737.10 # km (Moon Mean Radius)
a_moon=384399 %km 
    
%% Inputs:
% This section obtains the inputs from the user
% Parking Orbit Altitude in km from Departure Body Surface (h_parking)
h_parking=200 %km %input('Enter the Initial Parking Orbit Altitude(km): ');
% Target Body Mu (mu_target)
Mu_target=Mu_moon; %input('Enter the Gm or Mu of the Target (km^3/s^2): ');
% Target Body Mean Radius (r_target)
r_target=r_moon; %input('Enter the Mean Surface Radius of the target (km): ');
...
a_initial=0;  % Initial Body semimajor axis, in this case the Earth is also the central body
r_initial=r_terra;  % Initial Body Mean Equatorial Radius
Mu_initial=Mu_terra; % Initial Body Mu
Mu_central=Mu_terra; % Mu of the major central body for the transit orbit
...
V_initial=0; %set to 0, since the Earth is both the Initial and Central body
% Circular Orbit Velocity of the Initial Body (km/s)
V_target=sqrt(Mu_central/a_target); 
% Transit Orbit Velocity @ Initial Body (km/s)
V_plus_initial=sqrt(Mu_central*(1/a_transit)); 
